...
doCausalImpact <- function(zoo_data, intervention_date, ri.select=TRUE,time_points,crossval.stage=FALSE, var.select.on=TRUE, n_iter = 10000, trend = FALSE) {
  ...
  if(crossval.stage){
	...
	if(trend){
  	...
  	bsts_model.pois  <- poissonBvs(y=y.pre , X=x.pre, offset=offset.t.pre, BVS=var.select.on, model = list(deltafix=deltafix.mod,ri = FALSE))
	}else{
	  if(var.select.on){
	    deltafix.mod<-rep(0, times=(ncol(x.pre)))
	    deltafix.mod[1:(n_seasons-1)]<-1 #fix  monthly dummies
	    bsts_model.pois  <- poissonBvs(y=y.pre , X=x.pre, BVS=TRUE, model = list(deltafix=deltafix.mod,ri = FALSE))
	  }else{
	    if(ri.select){
	    bsts_model.pois  <- poissonBvs(y=y.pre , X=x.pre, BVS=FALSE, model = list(ri = FALSE))
	    }else{
