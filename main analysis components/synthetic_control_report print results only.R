...
if (run_pandoc) {
  rmarkdown::render('synthetic_control_report print results only.Rmd', output_file = 'Synthetic Control Report.html', output_dir = output_directory, params = param_list, envir = environment())	
} else {
  knitr::knit('synthetic_control_report print results only.Rmd', envir = environment())
  markdown::markdownToHTML('synthetic_control_report print results only.md', output = paste(output_directory, 'synthetic_control_report.html', sep = ''))
