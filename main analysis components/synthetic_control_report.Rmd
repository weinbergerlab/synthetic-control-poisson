...
if (crossval){
kable( cbind.data.frame(rr_mean_stack_intervals,rr_mean_allvars_intervals,rr_mean_full_intervals,rr_mean_time_intervals,rr_mean_time_no_offset_intervals, rr_mean_pca_intervals), align = 'c')
}else{
kable( cbind.data.frame(rr_mean_allvars_intervals,rr_mean_full_intervals,rr_mean_time_intervals,rr_mean_time_no_offset_intervals, rr_mean_pca_intervals), align = 'c')  
}
```
...
##Inclusion probabilities from Allvars model inclusion_prob_allvars
```{r incl_allvars, include = FALSE}
incl_probs <- NULL
	for (group in groups) {
	    incl_prob=impact_allvars[[group]]$inclusion_probs[-c(1:(n_seasons-1)),]
