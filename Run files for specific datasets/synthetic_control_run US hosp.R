...
country       <- 'US' #Country or region name.
...
input_directory  <- 'C:/Users/dmw63/Desktop/My documents h/GATES/CausalImpact code/Input data/US/' #Directory (or URL) containing input data file.
file_name="prelog_US_processed_data.csv"
...
outcome_name <- 'm_ACP'    #Name of column containing outcome.
denom_name   <- 'ach_sid_noresp'   #Name of column containing denominator to be used in offset.

#MOST DATES MUST BE IN FORMAT "YYYY-MM-01", exception is end of pre period, which is 1 day before end of post period
start_date        <- as.Date('1994-01-01') #Indicates the date of the first data point.
intervention_date <- as.Date('1999-12-31') #Indicates the date of intervention in the data.
end_date          <- as.Date('2004-12-01') #Indicates the date of the last data point.
pre_period        <- as.Date(c('1994-01-01', '1999-12-31')) #Range over which the data is trained for the CausalImpact model.
post_period       <- as.Date(c('2000-01-01', '2004-12-01')) #Range from the intervention date to the end date.
eval_period       <- as.Date(c('2002-01-01', '2004-12-01')) #Range over which rate ratio calculation will be performed.
